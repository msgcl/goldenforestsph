...
export default function Technology() {
  ...
  const copy = siteCopy?.technology ?? defaultSiteCopy.technology;
    ...
    <AnimatedPage>
      ...
      <div className="relative rounded-3xl overflow-hidden border border-[#2B6A55] bg-[#17392E] p-8 md:p-12 mb-16 shadow-2xl">
        ...
        <div className="relative z-10 max-w-3xl">
          <h2 className="text-3xl font-bold text-accent font-outfit mb-6">{copy.stackTitle}</h2>
            ...
            {copy.stackTitles.map((title, index) => {
              ...
              return (
                <div key={title} className="flex gap-4">
                  ...
                  <div>
                    <h4 className="text-slate-100 font-bold mb-1">{title}</h4>
                    <p className="text-slate-300 text-sm leading-relaxed">{copy.stackDescriptions[index] ?? ""}</p>
                  </div>
                </div>
              );
            })}
      ...
      <h3 className="text-2xl font-bold font-outfit mb-6">{copy.benefitsTitle}</h3>
